...
#        print(url,payload)
    ...
    def _create_update_payload(self, criteria, document):
        return "criteria=%s&newobj=%s" % (
            urllib.parse.quote( json.dumps(criteria) ),
            urllib.parse.quote( json.dumps(document) )
        )
...
#        print(response_object, type(response_object))
        if response_object:
            for result in response_object['results']:
                try:
#                    if result['_id'].find('-') == -1:
    ...
    def _create_find_get_params(self, criteria, fields=False):
        params = "?batch_size=9999999&criteria=%s" % urllib.parse.quote(
                json.dumps(criteria)
            )
        if fields:
            params = "%s&fields=%s" % (
    ...
    def _create_insert_payload(self, document):
        return "docs=[%s]" %  urllib.parse.quote( json.dumps(document) )
