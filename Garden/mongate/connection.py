...
class Connection(object):
    ...
    def _valid_response(self, response_object):
        ...
        if not 'ok' in response_object:
            return True
            
        return 'ok' in response_object and response_object['ok']
        
    def _batch_operation_ok(self, response_object):
        if len(response_object) > 0 and 'ok' in response_object[0] and not response_object[0]['ok']:
            return False
        return True
